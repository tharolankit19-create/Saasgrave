  ...
  return (
    <DropdownMenu.Root>
      <DropdownMenu.Trigger asChild>
        <button className="flex items-center gap-2 rounded-full border border-line p-1 pr-3 transition hover:border-ink/25">
          {avatarUrl ? (
            <Image
              src={avatarUrl}
              alt=""
          ...
          <span className="max-w-[120px] truncate text-sm text-ink-soft">{name || "Account"}</span>
        </button>
      ...
      <DropdownMenu.Portal>
        <DropdownMenu.Content
          align="end"
          sideOffset={8}
          className="z-50 min-w-[200px] rounded-xl border border-line bg-sunken p-1.5 shadow-2xl"
        >
          ...
          <DropdownMenu.Item asChild>
            <a
              href="/logout"
              className="flex items-center gap-2.5 rounded-lg px-3 py-2 text-sm text-ink-soft outline-none transition hover:bg-sunken hover:text-ink"
            >
